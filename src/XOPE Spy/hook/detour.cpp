...
void* Detour64::patch()
{
	...
	m_trampoline = new uint8_t[m_bytesToPatch + 14];

	MemoryProtect ignore(m_trampoline, m_bytesToPatch + 14, PAGE_EXECUTE_READWRITE, false);
	memcpy(m_trampoline, m_targetFunc, m_bytesToPatch);
	m_trampoline[m_bytesToPatch] = (uint8_t)0xFF; //JMP near absolute 
	m_trampoline[m_bytesToPatch + 1 ] = (uint8_t)0x25;
	*(uint32_t*)&m_trampoline[m_bytesToPatch + 2] = 0; //((_targetFunc - _trampoline - 5));
	*(uint64_t*)&m_trampoline[m_bytesToPatch + 6] = (uint64_t)m_targetFunc + m_bytesToPatch;

	MemoryProtect mp(m_targetFunc, m_bytesToPatch, PAGE_EXECUTE_READWRITE);
	m_targetFunc[0] = (uint8_t)0xFF; //JMP
	m_targetFunc[1] = (uint8_t)0x25; //[RIP]
	*(uint32_t*)&m_targetFunc[2] = 0;
