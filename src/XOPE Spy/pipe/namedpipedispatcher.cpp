...
bool NamedPipeDispatcher::send(std::unique_ptr<client::IMessage> mes)
{
    if (isPipeBroken())
        return false;
...
NamedPipeDispatcher::OutMessage_P NamedPipeDispatcher::serializeMessage(std::unique_ptr<client::IMessage> message)
{
    DWORD bytesWritten{ 0 };
    json j;
